...
**Omni-MATH 신규 실험은 [별도 분할 문서](OMNI_EVOLUTION_DESIGN.md)를 따른다.** 아래의 240/60 수치와 기본 실행 명령은 기존 GSM8K 실험용이다. Omni-MATH를 Modal에 보낼 때는 `--dataset omni_v1 --minibatch-size 5`와 새로운 `run-id`를 지정하고, 182문항 검증에 맞춰 평가 예산을 새로 정한다. 이 경우 미니배치는 자동으로 난이도별 1:3:1 추출을 사용한다.
...
풀이와 reflection에는 `temperature=0`을 사용한다. **새 실행의** 풀이 출력 한도는 16,384토큰, reflection 출력 한도는 4,096토큰, 모델 API 요청 읽기 제한은 1,800초다. 기존 `ko-main-b1200-m5-s0`는 2,048토큰·120초로 실행됐으며 [중단 조사](KO_MAIN_FAILURE.md)에 결과를 남겼다. 로컬 `.env`에 예전 제한이 있으면 `MOTIF_TIMEOUT_SECONDS=1800`으로 맞춘다. 인프론의 Motif 3 무료 제공은 앞서 확인한 공지상 **2026년 9월 29일까지**이므로 실행 직전에 모델 요금과 계정 한도를 다시 확인한다.
...
`--detach`를 사용하면 로컬 터미널이나 컴퓨터를 종료해도 원격 함수는 계속 실행된다. Modal의 함수 **한 번당 최대 실행 시간은 24시간**이며 코드도 `timeout=86400`으로 설정했다. 진화와 보류 평가 함수에는 최대 10회 자동 재시도를 설정했다. 시간 초과 후 새 시도는 같은 `run-id`로 Volume의 마지막 GEPA 상태에서 이어진다. 중간 상태는 `motif-gepa-ko-runs` Volume에 저장되며, GEPA가 상태를 저장할 때·각 반복이 끝날 때·함수가 종료될 때 명시적으로 commit한다. 진화 중 성공한 모델 응답도 5회마다 commit하고, 새 시도에서 동일 요청을 만나면 저장된 응답을 재사용한다. 강제 종료 시 마지막 commit 이후 최대 몇 건의 응답과 진행 중이던 반복은 다시 수행될 수 있다. 같은 ID로 설정·코드를 바꾸면 복구를 중지하므로 자동 재시도 중에는 실행 파일을 변경하지 않는다.

재시도는 시간 초과뿐 아니라 코드·API 오류에도 적용되므로 영구적인 외부 오류를 없애지는 못한다. 10회를 모두 소진하면 상태를 확인해 수동으로 같은 설정·ID로 재개해야 한다. `max-api-calls`는 **한 시도당 논리 호출 상한**이며, 여러 Modal 시도 전체를 합산한 상한은 아니다. `api_calls.json`에서 새 제공자 호출과 이전 응답 재사용 건수를 구분한다. `max-metric-calls`는 GEPA 저장 상태에 이어서 적용된다.

### 긴 진화 실행: 1,200회 예산·훈련 묶음 5문항

아래는 2026-09-25 중단된 **지난 실행의 명령 기록**이다. 예비 실행과 같은 train 240문항·val 60문항, 초기 프롬프트, 난수 시드 0을 사용하고 새 실행 ID에서 처음부터 탐색했다. 한 반복에서 부모 프롬프트로 훈련 문제 5개를 풀며, 제안이 생기면 같은 5개로 다시 평가한다. 전체 val 60문항 평가는 제안이 훈련 묶음 심사를 통과한 경우에만 수행한다. 이 설정은 예비 실행과 예산과 묶음 크기가 모두 다르므로 진화 경로 차이를 한 변수의 효과로 해석하지 않는다.
